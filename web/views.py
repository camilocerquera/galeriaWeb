...
from serializers import UrlSerialializer
...
def index(request):
    ...
    r = requests.get(os.environ.get('REST_URLS'))
    data = r.json()
    serializer = UrlSerialializer(data = data, many="true")
    if serializer.is_valid():
        tabla_urls = serializer.data
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (url !='' and title !='' and description != ''):
                ...
                #urlObject.save()
                parametros = {"url":url,"user":user, "date":now, "title":title,"description":description}
                r = requests.post(os.environ.get('REST_URLS'), parametros, auth=(os.environ.get('REST_USER'), os.environ.get('REST_PASSWORD')))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (name !='' and email !='' and msg != ''):
                ...
                #contactObject.save()
                parametros = {"name":name,"email":email, "msg":msg, "date":now}
                r = requests.post(os.environ.get('REST_CONTACTS'), parametros, auth=(os.environ.get('REST_USER'), os.environ.get('REST_PASSWORD')))
